...
protected:
	...
	HWND m_hWnd;
	// Original WndProc of parent window
	WNDPROC m_OriParentProc;

	enum ShadowStatus
	{
		// Shadow is enabled, 
		// if not, the following one is always false
		SS_ENABLED = 1,
		// Shadow window is visible
		SS_VISABLE = 1 << 1,
		// Parent window is visible, 
		// if not, the above one is always false
		SS_PARENTVISIBLE = 1<< 2
	};
	BYTE m_Status;

	// Darkness, transparency of blurred area
	unsigned char m_nDarkness;
	// Sharpness, width of blurred border of shadow window
	unsigned char m_nSharpness;
	// Shadow window size, relative to parent window size
	signed char m_nSize;

	// The X and Y offsets of shadow window,
	// relative to the parent window, 
	// at center of both windows (not top-left corner), signed
	signed char m_nxOffset;
	signed char m_nyOffset;

	// Restore last parent window size, 
	// used to determine the update strategy 
	// when parent window is resized
	LPARAM m_WndSize;

	// Set this to true if the shadow 
	// should not be update 
	// until next WM_PAINT is received
	bool m_bUpdate;
...
protected:
	...
	// called when window resized or shadow properties changed, 
	// but not only moved without resizing
