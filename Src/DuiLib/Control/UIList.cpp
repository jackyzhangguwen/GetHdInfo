...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
	...
	m_ListInfo.nFont = -1;
	// m_uTextStyle(DT_VCENTER | DT_END_ELLIPSIS)
...
bool CListUI::SetMultiItemIndex(CControlUI* pStartControl, 
	int iCount, 
	int iNewStartIndex)
{
...
bool CListUI::Add(CControlUI* pControl)
{
	...
	if (pControl->GetInterface(DUI_CTR_LISTHEADER) != NULL) {
		if (m_pHeader != pControl && 
			m_pHeader->GetCount() == 0) {
...
{
	if (m_ListInfo.diBk.sDrawString == pStrImage &&
		m_ListInfo.diBk.pImageInfo != NULL) {
		return;
	}
...
{
	if (m_ListInfo.diSelected.sDrawString == pStrImage &&
		m_ListInfo.diSelected.pImageInfo != NULL) {
		return;
	}
...
{
	if (m_ListInfo.diHot.sDrawString == pStrImage &&
		m_ListInfo.diHot.pImageInfo != NULL) {
		return;
	}
...
{
	if (m_ListInfo.diDisabled.sDrawString == pStrImage &&
		m_ListInfo.diDisabled.pImageInfo != NULL) {
		return;
	}
...
void CListUI::EnsureVisible(int iIndex)
{
	...
	if (pHorizontalScrollBar &&
		pHorizontalScrollBar->IsVisible()) {
...
{
	if (m_diNormal.sDrawString == pStrImage &&
		m_diNormal.pImageInfo != NULL) {
		return;
	}
	m_diNormal.Clear();
...
{
	if (m_diHot.sDrawString == pStrImage &&
		m_diHot.pImageInfo != NULL) {
		return;
	}
	m_diHot.Clear();
...
{
	if (m_diPushed.sDrawString == pStrImage &&
		m_diPushed.pImageInfo != NULL) {
		return;
	}
	m_diPushed.Clear();
...
{
	if (m_diFocused.sDrawString == pStrImage &&
		m_diFocused.pImageInfo != NULL) {
		return;
	}
	m_diFocused.Clear();
...
{
	if (m_diSep.sDrawString == pStrImage &&
		m_diSep.pImageInfo != NULL) {
		return;
	}
	m_diSep.Clear();
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
	...
	if ((pInfo->uTextStyle & DT_SINGLELINE) == 0 &&
		(szAvailable.cx != m_szAvailableLast.cx || 
			szAvailable.cy != m_szAvailableLast.cy)) {
		m_bNeedEstimateSize = true;
	}
	if (m_uFixedHeightLast != pInfo->uFixedHeight || 
...
void CListHBoxElementUI::SetPos(RECT rc, bool bNeedInvalidate)
{
	...
	if (pInfo->nColumns > 0) {
		...
		for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
			...
			RECT rcPadding = pControl->GetPadding();
			RECT rcItem = { 
				pInfo->rcColumn[iColumnIndex].left + rcPadding.left, 
				m_rcItem.top + rcPadding.top,
				pInfo->rcColumn[iColumnIndex].right - rcPadding.right, 
				m_rcItem.bottom - rcPadding.bottom };
			if (pInfo->iVLineSize > 0 && 
				iColumnIndex < pInfo->nColumns - 1) {
				rcItem.right -= pInfo->iVLineSize;
			}
...
bool CListHBoxElementUI::DoPaint(HDC hDC, 
	const RECT& rcPaint, 
	CControlUI* pStopControl)
{
	ASSERT(m_pOwner);
	...
	for (int i = 0; i < pInfo->nColumns; i++) {
		...
		if (pInfo->iVLineSize > 0 && 
			i < pInfo->nColumns - 1) {
			RECT rcLine = { 
				rcItem.right - pInfo->iVLineSize / 2, 
				rcItem.top, 
				rcItem.right - pInfo->iVLineSize / 2, 
				rcItem.bottom };
			CRenderEngine::DrawLine(hDC, rcLine, pInfo->iVLineSize, GetAdjustColor(pInfo->dwVLineColor));
