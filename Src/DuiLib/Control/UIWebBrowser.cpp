...
STDMETHODIMP CWebBrowserUI::GetTypeInfo( UINT iTInfo, 
	LCID lcid, 
	ITypeInfo **ppTInfo )
{
	return E_NOTIMPL;
...
STDMETHODIMP CWebBrowserUI::GetIDsOfNames( REFIID riid, 
	OLECHAR **rgszNames, 
	UINT cNames, 
	LCID lcid,
	DISPID *rgDispId )
...
STDMETHODIMP CWebBrowserUI::Invoke( DISPID dispIdMember, 
	REFIID riid, 
	LCID lcid,
	WORD wFlags, 
...
/// DWebBrowserEvents2
void CWebBrowserUI::BeforeNavigate2( 
	IDispatch *pDisp,
	VARIANT *&url,
	VARIANT *&Flags,
	...
	VARIANT_BOOL *&Cancel )
{
	if (m_pWebBrowserEventHandler) {
		m_pWebBrowserEventHandler->BeforeNavigate2(
			pDisp,
			url,
			Flags,
			TargetFrameName,
...
void CWebBrowserUI::NavigateError( 
	IDispatch *pDisp,
	VARIANT * &url,
	VARIANT *&TargetFrameName,
	VARIANT *&StatusCode,
...
{
	if (m_pWebBrowserEventHandler) {
		m_pWebBrowserEventHandler->NavigateError(
			pDisp,
			url,
			TargetFrameName,
			StatusCode,
...
{
	if (m_pWebBrowserEventHandler) {
		m_pWebBrowserEventHandler->NewWindow3(
			pDisp,
			Cancel,
			dwFlags,
			bstrUrlContext,
...
// IDownloadManager
STDMETHODIMP CWebBrowserUI::Download( 
	/* [in] */ IMoniker *pmk, 
	/* [in] */ IBindCtx *pbc, 
	/* [in] */ DWORD dwBindVerb, 
	/* [in] */ LONG grfBINDF, 
...
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->Download(
			pmk,
			pbc,
			dwBindVerb,
			grfBINDF,
...
// IDocHostUIHandler
STDMETHODIMP CWebBrowserUI::ShowContextMenu( 
	DWORD dwID, 
	POINT* pptPosition, 
...
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->ShowContextMenu(
			dwID,
			pptPosition,
			pCommandTarget,
			pDispatchObjectHit);
...
STDMETHODIMP CWebBrowserUI::ShowUI( 
	DWORD dwID, 
...
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->ShowUI(
			dwID,
			pActiveObject,
			pCommandTarget,
			pFrame,
...
STDMETHODIMP CWebBrowserUI::ResizeBorder( 
	LPCRECT prcBorder, 
	IOleInPlaceUIWindow* pUIWindow, 
	BOOL fFrameWindow )
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->ResizeBorder(
			prcBorder,
			pUIWindow,
			fFrameWindow);
	}
...
STDMETHODIMP CWebBrowserUI::TranslateAccelerator( 
	LPMSG lpMsg, 
	const GUID* pguidCmdGroup, 
	DWORD nCmdID )
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->TranslateAccelerator(
			lpMsg,
			pguidCmdGroup,
			nCmdID);
	}
...
LRESULT CWebBrowserUI::TranslateAccelerator( MSG *pMsg )
{
	...
	if (m_pWebBrowser2 == NULL) { return E_NOTIMPL; }
...
STDMETHODIMP CWebBrowserUI::GetOptionKeyPath( 
	LPOLESTR* pchKey, 
	DWORD dwReserved )
{
	if (m_pWebBrowserEventHandler) {
...
STDMETHODIMP CWebBrowserUI::GetDropTarget( 
...
STDMETHODIMP CWebBrowserUI::TranslateUrl( 
	DWORD dwTranslate, 
	OLECHAR* pchURLIn, 
	OLECHAR** ppchURLOut )
{
	if (m_pWebBrowserEventHandler) {
		return m_pWebBrowserEventHandler->TranslateUrl(
			dwTranslate,
			pchURLIn,
			ppchURLOut);
	} else {
...
STDMETHODIMP CWebBrowserUI::FilterDataObject( 
	IDataObject* pDO, 
	IDataObject** ppDORet )
{
	if (m_pWebBrowserEventHandler) {
...
void CWebBrowserUI::SetWebBrowserEventHandler( 
	CWebBrowserEventHandler* pEventHandler )
{
...
void CWebBrowserUI::SetAttribute( 
	LPCTSTR pstrName, 
	LPCTSTR pstrValue )
{
...
void CWebBrowserUI::NavigateUrl( LPCTSTR lpszUrl )
{
	if (m_pWebBrowser2 && 
		lpszUrl) {
		m_pWebBrowser2->Navigate(
			(BSTR)SysAllocString(T2BSTR(lpszUrl)),
			NULL,
			NULL,
			NULL,
			NULL);
...
STDMETHODIMP CWebBrowserUI::QueryService( 
	REFGUID guidService, 
	REFIID riid, 
	void** ppvObject )
{
...
HRESULT CWebBrowserUI::InvokeMethod( 
	IDispatch *pObj, 
	LPOLESTR pMehtod, 
	VARIANT *pVarResult, 
	VARIANT *ps, 
...
HRESULT CWebBrowserUI::GetProperty( 
	IDispatch *pObj, 
	LPOLESTR pName, 
	VARIANT *pValue )
{
...
HRESULT CWebBrowserUI::SetProperty( 
	IDispatch *pObj, 
	LPOLESTR pName, 
	VARIANT *pValue )
{
...
HRESULT STDMETHODCALLTYPE CWebBrowserUI::QueryStatus( 
...
HRESULT STDMETHODCALLTYPE CWebBrowserUI::Exec( 
	__RPC__in_opt const GUID *pguidCmdGroup, 
	DWORD nCmdID, 
	DWORD nCmdexecopt, 
	...
	__RPC__inout_opt VARIANT *pvaOut )
{
	...
	if (pguidCmdGroup && 
		...
		case OLECMDID_SHOWSCRIPTERROR:
		{
			...
			hr = pvaIn->punkVal->QueryInterface(
				IID_IHTMLDocument2, 
				(void **) &pDoc);    
