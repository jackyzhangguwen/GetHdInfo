...
void CLabelUI::PaintText(HDC hDC)
{
	...
	else
	{
		...
		LinearGradientBrush nLineGrBrushA(
			Point(GetGradientAngle(), 0), 
			Point(0, nGradientLength), 
			ARGB2Color(GetTextShadowColorA()), 
			ARGB2Color(GetTextShadowColorB() == -1 ? GetTextShadowColorA() : GetTextShadowColorB()));
		LinearGradientBrush nLineGrBrushB(
			Point(GetGradientAngle(), 0),
			Point(0, nGradientLength), 
			ARGB2Color(GetTextColor()),
			ARGB2Color(GetTextColor1() == -1 ? GetTextColor() : GetTextColor1()));
		...
		{
			LinearGradientBrush nLineGrBrushStroke(
				Point(GetGradientAngle(), 0),
...
#ifdef _UNICODE
		if (GetEnabledShadow() &&
			(GetTextShadowColorA() > 0 || 
				GetTextShadowColorB() > 0)) {
			nGraphics.DrawString(m_sText, 
				m_sText.GetLength(), 
				&nFont, 
				nShadowRc, 
				&format, 
