...
{
	if (m_diBkDisabled.sDrawString == pStrImage &&
		m_diBkDisabled.pImageInfo != NULL) {
		return;
	}
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_SETFOCUS ) { return; }
	if( event.Type == UIEVENT_KILLFOCUS ) { return;	}
